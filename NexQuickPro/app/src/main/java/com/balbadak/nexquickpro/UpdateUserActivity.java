...
public class UpdateUserActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        picture_button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                try {
                    if (ActivityCompat.checkSelfPermission(UpdateUserActivity.this, android.Manifest.permission.READ_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
                        ActivityCompat.requestPermissions(UpdateUserActivity.this, new String[]{android.Manifest.permission.READ_EXTERNAL_STORAGE, android.Manifest.permission.WRITE_EXTERNAL_STORAGE}, PICK_PICTURE_FROM_GALLERY);
                    } else {
            ...
            public void onClick(View v) {
                try {
                    if (ActivityCompat.checkSelfPermission(context, android.Manifest.permission.READ_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
                        ActivityCompat.requestPermissions(UpdateUserActivity.this, new String[]{android.Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.WRITE_EXTERNAL_STORAGE}, PICK_PICTURE_FROM_GALLERY);
                    } else {
    ...
    class PictureBackThread extends Thread {
        ...
        --경계문자--      => 첨부파일 전송 끝부분*/
                ...
                dos.writeBytes(qpPhone + lineEnd);
                ///여기까지 파라매터 추가하는 것
...
//
        ...
        if (usablePw && pwPaired) {
            ...
            UserInfoUpdateTask updateTask = new UserInfoUpdateTask(url, values);  //여기서 가입함....
            updateTask.execute();
...
*/
        ...
        protected void onPostExecute(String s) {
            ...
            if (s != null) {
                ...
                try {
                    ...
                    mUserNameView.setText(data.getString("qpName"));
                    mUserNameView.setEnabled(false);
                    mUserPhoneView.setText(data.getString("qpPhone"));
                    mUserPhoneView.setEnabled(false);
                    mUserBankView.setText(data.getString("qpBank"));
                    mUserAccountView.setText(data.getString("qpAccount"));

                    int vehicle = data.getInt("qpVehicleType");
                    switch (vehicle) {
                        case 1:
                            rg.check(R.id.vehicleBtn1);
                            break;
                        case 2:
                            rg.check(R.id.vehicleBtn2);
                            break;
                        case 3:
                            rg.check(R.id.vehicleBtn3);
                            break;
                        case 4:
                            rg.check(R.id.vehicleBtn4);
                            break;
                    }
    ...
    public class UserInfoUpdateTask extends AsyncTask<Void, Void, Boolean> {

        private String url;
        private ContentValues values;

        UserInfoUpdateTask(String url, ContentValues values) {
            this.url = url;
            this.values = values;
        }
        ...
        protected void onPostExecute(Boolean check) {
            super.onPostExecute(check);
               Toast.makeText(context, "정보 수정이 완료되었습니다.", Toast.LENGTH_LONG).show();
                finish();
        }
